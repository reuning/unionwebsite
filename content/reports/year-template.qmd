---
title: {{< template Year >}} Union Filing Report
description: Data on union election filings in {{< template Year >}}
...
filed <- dt |> filter( `Case_Type`=="RC")  |>
        filter(year(Date_Filed) == {{< template Year >}}) |> nrow()
closed <- dt |>  filter( `Case_Type`=="RC")  |>
        filter(year(Date_Closed) == {{< template Year >}}) |> nrow()
closed_success <- dt |>  filter( `Case_Type`=="RC")  |> 
        filter((year(Date_Closed) == {{< template Year >}}) &
                (Union_Cer=="Yes")) |> nrow()
...
year_dt <- dt |> filter(year(Date_Filed) == {{< template Year >}})
...
## Closed
```{r}
year_dt <- dt |> filter(year(Date_Closed) == {{< template Year >}})
