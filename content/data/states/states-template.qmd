...
open-graph: 
    title: {{< template State >}} Union Elections
    description: Data on recent union elections in {{< template State >}}.
---
...
#| label: unit-outcome-{{< template State >}}
...
```{r}
#| label: worker-outcome-{{< template State >}}
...
```{r}
#| label: newly-covered-{{< template State >}}
...
table_out <- data |> filter(Status=="Open") |> 
    ...
    mutate(NLRB=paste0("https://www.nlrb.gov/case/", Case)) |> 
    gt()

table_out |> 
    cols_label_with(  fn=function(x) gsub("_", " ", x)) |>
    fmt_url(columns=NLRB, label="(NLRB)") |> 
    cols_merge(
    columns = c(Case, NLRB),
    pattern = "{1} {2}"
  )
