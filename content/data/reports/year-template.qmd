---
title: 2023 Union Filing Report
...
filed <- dt |> filter( `Case_Type`=="RC")  |>
        filter(year(Date_Filed) == 2023) |> nrow()
closed <- dt |>  filter( `Case_Type`=="RC")  |>
        filter(year(Date_Closed) == 2023) |> nrow()
closed_success <- dt |>  filter( `Case_Type`=="RC")  |> 
        filter((year(Date_Closed) == 2023) &
                (Union_Cer=="Yes")) |> nrow()
...
year_dt <- dt |> filter(year(Date_Filed) == 2023)
...
## Closed
```{r}
year_dt <- dt |> filter(year(Date_Closed) == 2023)
