...
open-graph: 
    title: United Federation LEOS-PBA Union Elections
    description: Data on recent union elections in United Federation LEOS-PBA.
---
...
library(gt)
library(here)
data <- read_csv("United Federation LEOS-PBA_data.csv") |> filter(Case_Type == "RC") |> 
  mutate(Ballot_Type= 
...
#| label: unit-outcome-United Federation LEOS-PBA
...
```{r}
#| label: worker-outcome-United Federation LEOS-PBA
...
```{r}
#| label: newly-covered-United Federation LEOS-PBA
...
```{r}
#| label: timline-United Federation LEOS-PBA
