---
title: 2017 Union Filing Report
...
filed <- dt |> filter( `Case_Type`=="RC")  |>
        filter(year(Date_Filed) == 2017) |> nrow()
closed <- dt |>  filter( `Case_Type`=="RC")  |>
        filter(year(Date_Closed) == 2017) |> nrow()
closed_success <- dt |>  filter( `Case_Type`=="RC")  |> 
        filter((year(Date_Closed) == 2017) &
                (Union_Cer=="Yes")) |> nrow()
...
year_dt <- dt |> filter(year(Date_Filed) == 2017)
...
## Closed
```{r}
year_dt <- dt |> filter(year(Date_Closed) == 2017)
