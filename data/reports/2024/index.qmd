---
title: 2024 Union Filing Report
...
filed <- dt |>
        ...
        filter(year(Date_Filed) == 2024) |> nrow()
closed <- dt |>  
        filter(Case_Type == "RC" | (Case_Type=="RM" & Date_Filed > as.Date("2023-08-25"))) |> 
        filter(year(Date_Closed) == 2024) |> nrow()
closed_success <- dt |>  
        filter(Case_Type == "RC" | (Case_Type=="RM" & Date_Filed > as.Date("2023-08-25")))   |> 
        filter((year(Date_Closed) == 2024) &
                (Union_Cer=="Yes")) |> nrow()
median_unit <- dt |>  
        filter(Case_Type == "RC" | (Case_Type=="RM" & Date_Filed > as.Date("2023-08-25")))  |> 
        filter((year(Date_Closed) == 2024) &
...
year_dt <- dt |> filter(year(Date_Filed) == 2024)
...
## Closed
```{r}
year_dt <- dt |> filter(year(Date_Closed) == 2024)
