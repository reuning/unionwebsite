---
title: 2025 Union Filing Report
description: Data on union election filings in 2025
...
filed <- dt |>
        ...
        filter(year(Date_Filed) == 2025) |> nrow()
closed <- dt |>  
        filter(Case_Type == "RC" | (Case_Type=="RM" & Date_Filed > as.Date("2023-08-25"))) |> 
        filter(year(Date_Closed) == 2025) |> nrow()
closed_success <- dt |>  
        filter(Case_Type == "RC" | (Case_Type=="RM" & Date_Filed > as.Date("2023-08-25")))   |> 
        filter((year(Date_Closed) == 2025) &
                (Union_Cer=="Yes")) |> nrow()
median_unit <- dt |>  
        filter(Case_Type == "RC" | (Case_Type=="RM" & Date_Filed > as.Date("2023-08-25")))  |> 
        filter((year(Date_Closed) == 2025) &
...
year_dt <- dt |> filter(year(Date_Filed) == 2025)
...
## Closed
```{r}
year_dt <- dt |> filter(year(Date_Closed) == 2025)
