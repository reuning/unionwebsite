---
title: 2013 Union Filing Report
...
filed <- dt |> filter( `Case_Type`=="RC")  |>
        filter(year(Date_Filed) == 2013) |> nrow()
closed <- dt |>  filter( `Case_Type`=="RC")  |>
        filter(year(Date_Closed) == 2013) |> nrow()
closed_success <- dt |>  filter( `Case_Type`=="RC")  |> 
        filter((year(Date_Closed) == 2013) &
                (Union_Cer=="Yes")) |> nrow()
...
year_dt <- dt |> filter(year(Date_Filed) == 2013)
...
## Closed
```{r}
year_dt <- dt |> filter(year(Date_Closed) == 2013)
