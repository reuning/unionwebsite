---
title: United States Union Elections
...
unit_data <- plot_data |> group_by(Tally_Quarter, Union_Cer) |> 
...
ggplot(unit_data, aes(x=Tally_Quarter,
    ...
    scale_fill_viridis_d("Voted for Union?",
                        direction = -1,
                        begin = .15,
                        end=.85) +
...
gt(table_out) |> 
  ...
  tab_spanner( "By Unit", columns=c(Unionized, `Voted Against`)) |> 
...
activity_data <- plot_data |> 
    ...
    summarize(n=n()) |> 
    ungroup() |> 
    mutate(size=factor(size, labels=c("<5", "6-10", "11-25", "26-50", "51-100", "101-500", "500>"))) |> 
...
The below chart shows the number of filings in each month over the last 10 years. The current year's filings are in red. Note, it drops the _current_ month in order, but even with that there can be delays in filing processing. 
...
Month_Data <- plot_data |> 
    group_by(Month=month(Date_Filed), Year=year(Date_Filed)) |> 
    summarize(n=n()) |> 
    ungroup() |> 
    filter(Year>2010) |> 
    filter(!(Year == year(today()) & Month == month(today()))) |> 
    mutate(Current = Year==year(today()))

ggplot(Month_Data, aes(x=Month, y=n, group=Year, color=Current, alpha=Current)) + 
    geom_line() + 
    scale_color_manual(values=c("FALSE"="black", "TRUE"="red"))+ 
    scale_alpha_manual(values=c("FALSE"=.5, "TRUE"=1)) + 
    guides(color="none", alpha="none") + 
    labs(y="Number of Filings") + 
    scale_x_continuous(breaks = c(1,4,7,11), 
    labels=c("January", "April", "July", "November"))
